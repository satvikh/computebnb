...
export function TopStatusBar({ state }: { state: WorkerState }) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-[24px] border border-white/10 bg-black/25 px-4 py-4 shadow-[0_18px_55px_rgba(0,0,0,0.22)] backdrop-blur">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-xs text-zinc-300">
      ...
      <div className="flex flex-wrap items-center gap-4 text-xs text-slate-300">
        ...
        <span className="inline-flex items-center gap-1.5">
          ...
          {state.metrics.batteryPercent.toFixed(0)}% {state.machine.charging ? "charging" : "battery"}
        </span>
        <span className="inline-flex items-center gap-1.5">
